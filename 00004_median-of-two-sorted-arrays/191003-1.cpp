...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
		...
		int p2 = 0, q2 = n2, k2 = m - k1;
		for (;;) {
			if (k1 > 0 && k2 < nums2.size() && nums1[k1 - 1] > nums2[k2]) {
				int s = (q2 - k2) / 2;
				if (s > k1 - p1) s = k1 - p1;
				if (s == 0) s = 1;
				q1 = k1 - 1; p2 = k2 + 1;
				k1 -= s; k2 += s;
				continue;
			} else if (k2 > 0 && k1 < nums1.size() && nums2[k2 - 1] > nums1[k1]) {
				int s = (q1 - k1) / 2;
				if (s > k2 - p2) s = k2 - p2;
				if (s == 0) s = 1;
				p1 = k1 + 1; q2 = k2 - 1;
		...
		} else if (k1 < nums1.size()) {
			v = nums1[k1];
		} else if (k2 < nums2.size()) {
			v = nums2[k2];
		} else {
			v = 0;
		}
